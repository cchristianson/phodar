...
/* ---------- camera / pixel math ---------- */
function angSizeFromPoints(p1, p2, natW, natH, fovH) {
  if (!p1 || !p2 || !natW || !fovH) return null;
  const f = (natW / 2) / Math.tan((fovH * D2R) / 2);
...
function pixelDeltaAzEl(pA, pB, natW, fovH) {
  const f = (natW / 2) / Math.tan((fovH * D2R) / 2);
...
/* Direction of a pixel through the best available camera model.
   With a placed photo (mediaAim) the pose is exact. Without one, we build a
...
function pixelDirFromAnchor(px, py, refPx, refPy, refAz, refEl, natW, natH, fovH, mediaAim) {
  const fpx = (natW / 2) / Math.tan((fovH * D2R) / 2);
  if (mediaAim && isNum(mediaAim.az) && isNum(mediaAim.el)) {
    const b = photoBasis(+mediaAim.az, +mediaAim.el, +mediaAim.roll || 0);
...
const fmtDeg = (d) => (isFinite(d) ? `${(+d).toFixed(2)}°` : "—");
...
/* ============================================================
   ANALYSIS
   ============================================================ */
function analyze(sources, sigmaDeg = 1) {
...
function arbitrateBearings(sources) {
  const obs = sources
...
function analyzeTracks(sources) {
...
function sunPos(ms, lat, lng) { const lw = RAD * -lng, phi = RAD * lat, d = toDays(ms), c = sunCoordsC(d), H = sidereal(d, lw) - c.ra; return { az: (azimC(H, phi, c.dec) * R2D + 180 + 360) % 360, alt: altdC(H, phi, c.dec) * R2D }; }
function moonPos(ms, lat, lng) { const lw = RAD * -lng, phi = RAD * lat, d = toDays(ms), c = moonCoordsC(d), H = sidereal(d, lw) - c.ra; let h = altdC(H, phi, c.dec); h += refractionC(h); return { az: (azimC(H, phi, c.dec) * R2D + 180 + 360) % 360, alt: h * R2D }; }
function moonFrac(ms) { const d = toDays(ms), s = sunCoordsC(d), m = moonCoordsC(d), sd = 149598000; const phi = Math.acos(Math.sin(s.dec) * Math.sin(m.dec) + Math.cos(s.dec) * Math.cos(m.dec) * Math.cos(s.ra - m.ra)); const inc = Math.atan2(sd * Math.sin(phi), m.dist - sd * Math.cos(phi)); return (1 + Math.cos(inc)) / 2; }
...
/* --- homography: map the photo's flat quad onto the sky view exactly.
   ...
   gnomonic view is a projective transform → CSS matrix3d. --- */
function solveN(A, b) {
...
const dirToAzEl = (d) => ({ az: ((Math.atan2(d[0], d[1]) * R2D) + 360) % 360, el: Math.asin(clampN(d[2], -1, 1)) * R2D });
...
const compass8 = (h) => ["N", "NE", "E", "SE", "S", "SW", "W", "NW"][Math.round(h / 45) % 8];

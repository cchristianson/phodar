...
import { initTracker, stepTracker, stepObject, snapToObject, pinStep, smearDrift, despikePath, smoothPath, smoothObjPath, smoothPathAt, smoothObjPathAt, posePathAt, applyPoseFixes, applyDirFixes, snapDirsToAnchors } from "./video/postrack.js";
  ...
  },
  {
    ...
    groups: [
      ...
      { h: "Lay down the path & references (the tool row)", items: [
        ...
        { t: "⌖ Snap to object", d: "In ✎ Adjust, moves the selected point onto the fitted 3D object and anchors the WHOLE path to its measured direction. Use it on whichever point matches the moment you photographed — the object may have been anywhere along its path when the shutter fired, and this is what ties the traced path to the one direction you actually measured." },
      ...
      { h: "Trimming the clip (✂)", items: [
        { t: "Why trim", d: "The whip-pan while the phone comes up, and the blurred tail at the end, are exactly the frames that break the stabilizer. Cutting them off is a real quality lever, not tidiness — only the kept span is analysed, played, exported and reported." },
        { t: "Nothing is deleted", d: "The pixels are never re-encoded or thrown away. A trim just narrows what Phodar SAMPLES, so it is free, reversible, and can't lose footage. ⤢ full puts the whole clip back at any time." },
        { t: "Drag either end", d: "Grab a handle like the iPhone's own video editor — or press ANYWHERE on the bar (including the dimmed, trimmed-away part) and the nearest end jumps to your finger. That's how you pull a clipped end back out. The white line is the playhead." },
        { t: "⟨ start · end ⟩", d: "Set the edge exactly at the frame you're on, for when a drag isn't precise enough." },
        { t: "“The stabilization only covers …”", d: "A solved camera path describes the span it was solved over. If you widen the trim afterwards, playback and export still stop where the SOLVE stops — nothing was lost from the file; re-stabilize in the sky view to cover the new span." },
      ]},
      { h: "Shooting in-app (📷 Capture with sensors)", items: [
        { t: "What it's for", d: "A normal camera photo has no up/down angle and often no bearing. Recording inside Phodar logs the phone's attitude (up-angle, roll, heading) ~25× a second alongside the video, which gives the stabilizer real motion to lean on where the picture alone goes weak." },
        { t: "🎬 Record with motion data", d: "Starts the camera plus the attitude log. ⏹ Stop finishes it. Phodar syncs the log to the video's own clock automatically." },
        { t: "⚡ Quick frame (lower-res)", d: "Grabs a single still from the live preview at once — when the thing is happening NOW and a video isn't what you want. It's the preview's resolution, not the camera's full sensor, so use the normal Camera app if you have the seconds to spare." },
        { t: "🌙 Night / long exposure → 📁 Attach the shot", d: "iOS only gives web apps a basic camera, so Night mode lives in the Camera app. This FREEZES your current aim (bearing + up-angle, shown as “Aim frozen ✓”); you switch to the Camera app, shoot from the same spot at the same aim, come back and 📁 Attach the shot — the frozen pose is carried onto that photo." },
        { t: "⇅ flip tilt", d: "Flips the sign of the up/down angle if the phone reports it inverted. Compare the live readout against reality — pointing at the ground should read negative — and tap this if it's backwards." },
        { t: "📍 No position — tap to allow", d: "A recorded clip carries no EXIF, so without a GPS fix here you'll have to type your location on step 2. Tapping re-requests permission, which is what makes iOS show the prompt again after it was dismissed." },
        { t: "The trade-off, stated plainly", d: "The in-browser camera is ~1080p with no lens switching and no optical zoom. This is a MEASUREMENT mode — for the best-looking evidence, shoot in the normal Camera app and load that file instead." },
        { t: "👁 clean view", d: "Strips the screen to the crosshair, a one-line pose readout and the shutters, so you can actually watch the sky. Warnings that change what the capture is WORTH — camera errors, “no motion data”, missing position — deliberately survive." },
        { t: "Save it out", d: "An in-app recording lives only in Phodar's storage until you use 💾 Save to camera roll. Clearing site data would otherwise destroy the footage, so an unsaved clip is nagged." },
      ]},
  ...
  },
  {
    ...
    groups: [
      ...
      { h: "The map", items: [
        ...
        { t: "✥ Move", d: "While lit, the pin follows the map centre so dragging repositions you. Tap it off to pan and zoom around freely without moving your spot." },
  ...
  const exportStabilized = async (mode) => {
    ...
    try {
      ...
      /* PIXEL-PINNED FOLLOW (close-up export only): the track is a solve at
         ...
         touched. */
      /* v3 — the DECISION POLICY moved to pinStep (postrack.js, pure,
         mathcheck-asserted); this wrapper only owns the pixels (canvas grab
         from the seeked video). v2 failed in the field (object wandered ±20%
         of the close-up and left the frame): its clutter gate compared every
         find against the TRACK — the thing the pin exists to correct — so
         ~1° of track error rejected the pin's own correct finds, and each
         rejection eased the camera back toward the bad track. pinStep trusts
         the locked chain (the tight window is the gate, a 4× backstop bounds
         runaway), world-holds brief fades, and glides — never snaps — back
         to the track after a release. Reproduced + fixed in a harness driven
      ...
      const refinePin = (p, pred) => {
        ...
        const r = pinStep(pinSt, pred, { natW, natH, pose: p, maxAng: maxAngX }, (cx, cy, W2) => {
          ...
          try {
            ...
            pinCtx.drawImage(v, (cx - W2 / 2) * sv, (cy - W2 / 2) * sv, W2 * sv, W2 * sv, 0, 0, W2, W2);
            return pinCtx.getImageData(0, 0, W2, W2).data;
          } catch (e) { return null; }
        });
        return { az: r.az, el: r.el };
      };
      ...
      if (blob) {
        ...
        const pinNote = camFollow && pinTry ? ` · pixel-pinned on the object (${Math.round(100 * pinSt.ok / pinTry)}% of frames re-locked)` : "";
...
function PositionEditor({ src, update, others, viewOnly }) {
  ...
  useEffect(() => {
    ...
    let skyLo = -0.5, skyHi = 2;
    const el = (az) => skylineElAt(horiz.els, az);
    for (let az = 0; az < 360; az += 0.5) { const e = el(az); if (e < skyLo + 0.5) skyLo = e - 0.5; if (e > skyHi - 1) skyHi = e + 1; }
    /* ISOTROPIC mapping — the SAME px/° on both axes. The first version
       stretched the y-scale to contain the whole camera box, which squashed
       a portrait photo's 📷 overlay to a fraction of its height (field
       report: "stretched out"). Equal scales keep the overlay aspect-true
       AND degree-true at once; the visible elevation window is what the
       strip height affords, SLID by the tilt slider (the field ask) and
       clamped so the ridge — the matching anchor — never leaves the frame.
       The camera box clips at the window edges instead of inflating them.
       Bearing pans still translate only (scale is fixed by fovH alone). */
    const pxDeg = cssW / span;
    const rangeV = cssH / pxDeg;
    const boxTop = tilt + fovV / 2, boxBot = tilt - fovV / 2;
    let elBot = clampN(tilt - rangeV / 2, skyHi + 1 - rangeV, skyLo);
    if (skyHi + 1 - rangeV > skyLo) elBot = (skyHi + 1 - rangeV + skyLo) / 2; // ridge taller than the window — centre it
    const elTop2 = elBot + rangeV;
    const Y = (e) => cssH - (e - elBot) * pxDeg;
    const camBox = (Ymap) => {
    ...
    if (horiz3d) {
      ...
      const Y2 = Y;   // one isotropic mapping for both modes
      ...
      for (const bg of bldgs) {
        ...
        const w = Math.max(1.5, x2 - x1), y0 = Y2(0), y1 = Y2(Math.min(bg.elTop, elTop2 - 0.2));
      ...
      if (credit) {
        /* Esri requires visible attribution wherever its pixels appear;
           bottom-right, since the 📷 hold pill owns the top-right corner */
        ctx.textAlign = "right"; ctx.fillStyle = "rgba(255,255,255,.6)"; ctx.font = "7px ui-monospace, monospace";
        ctx.fillText("© Esri, Maxar, Earthstar Geographics", cssW - 3, cssH - 12);
      }
      return;
    }
    ...
    for (const bg of bldgs) {
      const x1 = X(b + bg.rel - bg.half), x2 = X(b + bg.rel + bg.half);
      const w = Math.max(1.5, x2 - x1), y0 = Y(0), y1 = Y(Math.min(bg.elTop, elTop2 - 0.2));
                  ...
                  {horizOn && (
                    <div style={{ position: "absolute", left: 0, right: 0, bottom: 0, height: 96, zIndex: 900, background: "#08101F", borderTop: "1px solid var(--line)" }}>
                      ...
                      {/* pills INSIDE the strip so they can't collide with
                          Leaflet controls or the lifted attribution — mode on
                          the left, 📷 hold on the far right (field ask); the
                          3D-mode Esri credit lives bottom-right to stay clear */}
                      ...
                      {src.mediaUrl && (
                        <button
                          ...
                          style={{ position: "absolute", right: 6, top: 6, zIndex: 5, fontSize: 10, padding: "2px 8px", borderRadius: 10, background: peek ? "rgba(58,43,16,.92)" : "rgba(10,15,28,.85)", color: peek ? "var(--amber)" : "#9ab", border: `1px solid ${peek ? "var(--amber)" : "rgba(255,255,255,.25)"}`, touchAction: "none", WebkitUserSelect: "none", userSelect: "none" }}>
                          📷 hold</button>
                      )}
                    </div>
                  )}
...
Rules: report numbers exactly as Phodar returns them; say plainly that the sky placement is approximate until a human refines it in the app (star or terrain alignment); and always end with the bundle link so I can import it at https://phodar.app to review and refine every measurement myself.

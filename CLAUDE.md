...
- **⚑ Shadow check: DONE (v2)** (`src/shadow.js`, pure + mathcheck-asserted):
  ...
  up = further, 6 m floor / 300 m cap — placement is a tilt, not a
  setting; follows the pan; sits under the photo in place mode) and
  draws the sighting-time shadow — direction exact, length assumes flat
  ground (stated in the status line). `shadowCast` arbitrates the light
  source: sun, else the moon when up (bright moon ⇒ faint blue shadow
  with % lit stated; <30% lit ⇒ honestly "too dim", nothing drawn),
  else nothing — and the line says ANY visible shadow contradicts the
  stated time/place. The report's Authenticity section carries the same
  geometry per observer as a compass-rose SVG (arrow = shadow, ☀/🌙 at
